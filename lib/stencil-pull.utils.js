...
utils.mergeThemeConfiguration = async (options) => {
    console.log("Start 'mergeThemeConfiguration'");
    if (!options) {
    ...
    console.log(JSON.stringify(localConfig.variations));
    let diffDetected = false;

    if (!remoteThemeConfiguration) {
        console.error("'remoteThemeConfiguration' is undefined");
    } else {
        console.log("'remoteThemeConfiguration' pass");
    }

    if (!remoteThemeConfiguration.settings) {
        console.error("'settings' is undefined");
    } else {
        console.log("'settings' pass");
    }

    const themeEntries = Object.entries(remoteThemeConfiguration.settings);

    console.log("'themeEntries.length' = " + themeEntries.length);
    ...
    let i = 0;
    try {
        for (const [key, remoteVal] of themeEntries) {
            i++;
            console.log(`index: ${i}; Key: ${key}; RemoteVal: ${remoteVal}`);
            if (!localConfig) {
                console.error("'localConfig' is undefined");
            } else {
                console.log("'localConfig' pass");
            }
            ...
            if (!_.isEqual(defaultVal, remoteVal)) {
                ...
                console.log("Setting 'variations.settings[key]'");
